...
let createauthor = async function (req, res) {
    try {
        ...
        if (!check.email) 
        { return res.status(400).send({ status: false, msg: "pls enter email" }) }
        if (!check.password)
         { return res.status(400).send({ status: false, msg: "pls enter passowrd" }) }
...
const login = async function (req, res) {
    ...
    if (!email) 
    { return res.status(400).send({ status: false, msg: "Add Email id" }) }
    if (!password) 
    { return res.status(400).send({ status: false, msg: "Add Password" }) }
    let get = await authormodel.findOne({ email: email, password: password })
    if (!get) 
    { return res.status(400).send({ status: false, msg: "your email and password is incorrect" }) }
